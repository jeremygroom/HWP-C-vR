    ...
    fluidRow(
      column(8, offset = 2, 
             box(width = 12,
                 ...
                 p(strong("Emissions are for the carbon content of the harvested wood only and do not include carbon emitted in the 
                        form of other carbon-containing greenhouse gasses, such as methane.", style = "font-size:20px")),
                 p(strong("The sum of the cumulative storage and emissions categories equals the cumulative harvest value for the previous 
                        year (compare these figures against the cumulative values for the Annual Timber Harvest figure).", style = "font-size:20px")),
                 p(strong("The wood products sector is determined to be a net source or sink using the IPCC Production Approach or 
                          IPCC Simple Decay Approach; see figures for Annual Net Change in Carbon Storage.", style = "font-size:20px"))
             )     
      ))
        ...
        insertUI(                                    # NOT renderUI.  This is better for having added and then taken away again (see below).
          ...
          textInput(NS(id, "change_title_csem"), label = h3("Enter figure title"), 
                    value = switch(as.numeric(input$filltype),
                                   "Cumulative total carbon in harvested wood product storage pools and emission categories",
                                   "Cumulative total carbon in harvested wood product storage pools and emission categories",
...
#browser()      
      ...
      p.e.1_po <- p.e.1 %>% filter(cFates %in% c("pu", "lf.fx", "lf", "dmp", "recov"))  # suffix "po" is for "pool" and "em" is for "emission"
      p.e.1_po$cFates <- p.e.1_po$cFates[drop = TRUE]  # Drops unused levels
      p.e.1_em <- p.e.1 %>% filter(cFates %in% c("comp", "dec", "fuel", "bwoec", "recov.decay", "dmp.decay", "lf.decay"))
      p.e.1_em$cFates <- p.e.1_em$cFates[drop = TRUE]  # Drops unused levels
      
      n.levs <- length(c(levels(p.e.1_po$cFates), levels(p.e.1_em$cFates)))
      p.e.levels <- c(levels(p.e.1_po$cFates), levels(p.e.1_em$cFates))    # Needed below for finalizing data set sent to plotting.
      #browser()
      hex_codes.p.e <- viridis_pal(option = "C")(n.levs * 4)  # Setting up Viridis colors to use in figure, flexible by number of groups.
      hex_codes.p.e.use <- hex_codes.p.e[seq(2, (n.levs * 4), 4)]  #  Trying to get colors that are somewhat different from one another by spacing them out a bit.
      hex_codes.p.e.use2 <- c(hex_codes.p.e.use[5:1], rev(hex_codes.p.e.use[6:n.levs]))  # Trying to reverse emissions colors for contrast
      ...
      "E:Fuel wood", "E:Discard energy capture", "E:Dump decay", "E:Landfill decay", "E:Recovery decay", "E:Compost decay", "E:Burned without\nenergy capture")
      
      
      legend.breaks <- c("dmp", "lf", "lf.fx", "recov", "pu", #"bwoec", "comp", "recov.decay", "lf.decay",  "dmp.decay", "dec",  "fuel" ) 
                          "fuel", "dec", "dmp.decay", "lf.decay", "recov.decay", "comp", "bwoec")
      if (n.levs == 11) {   # If there is no "burned with energy capture" then it is removed from the mix
        legend.labels <- legend.labels[-which(legend.labels == "E:Discard energy capture")]
        legend.breaks <- legend.breaks[-which(legend.breaks == "dec")]
      }
      ...
      if (is.null(p.e.rem) == F) {  # Reducing inputs if user selected to not show categories
        ...
        hex_codes.p.e.use2 <- hex_codes.p.e.use2[-p.e.rem]
      ...
      hl.names <- names(hl.data)[c(2:7, 11, 8:10)]
      ...
      out.data <- list(hwp.data, p1G.1, p1G.leg.names, p1G.breaks, p1G.colors, max.y.1, p.e.1, max.y.2, legend.labels, legend.breaks, hex_codes.p.e.use2,
      ...
      names(out.data) <- c("hwp.data", "p1G.1", "p1G.leg.names", "p1G.breaks", "p1G.colors",  "max.y.1", 
                           "p.e.1", "max.y.2", "legend.labels", "legend.breaks", "hex_codes.p.e.use2", 
                           "hl.data2", "hl.leg.names", "hl.names", "hex_codes.hl.use", "hl.vec", "max.y.3" )
      return(out.data)
    ...
    plotForOutput <- reactive({
     ...
     # browser()
      ...
      yr.sep <- round(length(seq(hwp.dataX$MIN.PLOT.YR, hwp.dataX$MAX.PLOT.YR, by = 10))/10, ) * 10
      ...
      if (input$filltype == "1") {
        ...
        gg.x <- ggplot() +
          ...
          scale_x_continuous(breaks = seq(hwp.dataX$MIN.PLOT.YR, max(plots.data()$p1G.1$years), by = yr.sep), limits = c(hwp.dataX$MIN.PLOT.YR, max(plots.data()$p1G.1$years)), minor_breaks = NULL) +
          ...
          labs(y = y.lab.cc, x = NULL, 
               title = if (input$action == 0) wrapper("Cumulative total carbon in harvested wood product storage pools and emission categories", 70) else wrapper(input$change_title_csem, 70)) + 
          theme_bw()  + 
        ...
        #browser()
          ...
          geom_hline(yintercept = 0) +
          scale_x_continuous(breaks = seq(hwp.dataX$MIN.PLOT.YR, hwp.dataX$MAX.PLOT.YR, by = yr.sep), limits = c(hwp.dataX$MIN.PLOT.YR, max(plots.data()$p.e.1$years)), minor_breaks = NULL) +
          ...
          scale_fill_manual(labels = plots.data()$legend.labels,
                            ...
                            values = plots.data()$hex_codes.p.e.use2, drop = FALSE) +  
          guides(fill = guide_legend(title = "(S)torage and (E)mission\nCategories"#element_blank()#,
          ),#nrow = 3, byrow = T), 
          override.aes = list(size = 1), reverse = T) +
          labs(y = y.lab.cc, x = NULL, 
               title = if (input$action == 0) wrapper("Cumulative total carbon in individual harvested wood product storage pools and emissions categories", 70) else wrapper(input$change_title_csem, 70)) + 
          theme_bw()  + 
          theme(text = element_text(size = 20), 
                legend.position = "right", legend.text = element_text(size = 15), legend.spacing.y = unit(0, 'cm'))
     ...
     # if (length(plots.data()$hl.vec) == 3) browser()
          ...
          geom_hline(yintercept = 0) +
          scale_x_continuous(breaks = seq(hwp.dataX$MIN.PLOT.YR, hwp.dataX$MAX.PLOT.YR, by = yr.sep), limits = c(hwp.dataX$MIN.PLOT.YR, max(plots.data()$hl.data2$years)), minor_breaks = NULL) +
